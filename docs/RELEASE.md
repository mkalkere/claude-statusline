...
2. **`TestRealSettingsUntouched`** snapshots a hash of the real file before any test runs (`setUpModule`) and asserts it is unchanged afterward — the regression guard that would have caught the original incident.
