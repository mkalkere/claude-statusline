...
When investigating a "Line 2 disappeared" report: (1) run `claude-status --doctor` and check `Columns:` reflects the user's real terminal width; (2) check whether OSC 8 links are enabled (their invisible escape bytes inflate measured width); (3) ask what version of Claude Code — fixes after 2.1.140 may have partially addressed the upstream bug.
