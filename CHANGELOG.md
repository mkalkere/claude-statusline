...
### Fixed
- **Width detection for Claude Code 2.1.139+** ([#83](https://github.com/mkalkere/claude-statusline/issues/83)) — 2.1.139 (2026-05-08) shipped "hooks now run without terminal access," which removed the last TTY-based escape hatch the v0.5.7 fallback chain relied on. The headline failure: on Linux/macOS, `tput cols` no longer fails when the subprocess has no TTY — it confidently returns its terminfo default (80 for `xterm`/`xterm-kitty`/`xterm-256color`/`vt100`/`ansi`). That value passed our `[20, 4000]` plausibility range, so we rendered an 80-col layout into the user's real (often 120–220 col) terminal. Independently confirmed by Node, Go, and PowerShell statusline authors in [anthropics/claude-code#22115](https://github.com/anthropics/claude-code/issues/22115) on 2026-05-12. Three defenses added:
  ...
  - **`COLUMNS=0` rejected and reported distinctly from unset.** 2.1.139 sometimes sets `COLUMNS` to "0" rather than leaving it absent; treating that as a real value would have returned 0 from step 2 (which then fails our `>= 20` guard but only after silently shaping subsequent diagnostics). The report distinction matters for `--doctor`.
  - **Process-tree walk added as a new fallback step.** Linux walks `/proc/<ppid>/fd/2` up the ancestor chain looking for a process that still owns the controlling terminal (the user's shell or Claude Code's main TUI process). Independently shipped by the Go statuslines [ccstatus](https://github.com/moond4rk/ccstatus) and [cc-tools](https://github.com/joshsymonds/cc-tools) as their primary 2.1.139 workaround. macOS lacks the equivalent `/proc/<pid>/fd` exposure so the walk degrades to checking PPID then bailing — that's acceptable: the user gets the safe fallback width, same as before this step existed. Capped at 16 ancestors and protected by a visited set to defuse pathological process trees.

### Added
- **`activity` section** ([#84](https://github.com/mkalkere/claude-statusline/issues/84)) — live tool-call counter for the *current assistant turn*, distinct from the existing `tools:` section (which is session-cumulative). Renders as `act:3` and is hidden when zero so idle sessions show nothing. Reads the tail of `transcript_path` from stdin (64 KiB cap), counts `tool_use` content blocks on assistant messages since the most recent `role: "user"` line, caches the result for 5 seconds. Tolerates: missing file, empty file, malformed JSON lines (skipped individually), partial first line when reading from a tail offset (discarded), no user message in the tail window (returns 0 rather than misleadingly counting a previous turn's activity), file rotation/truncation mid-session, and non-string `transcript_path` from upstream schema changes. Opt-in via custom theme — not added to the default layout to avoid silently changing every user's statusline. This is the "live activity" signal popularized by Claude HUD that several competitor tools surface; we now expose it without requiring a daemon, jq, or any network access.
- **`cache_hit` section** ([#84](https://github.com/mkalkere/claude-statusline/issues/84)) — prompt cache hit ratio: of the cacheable input, how much actually hit the cache. Renders as `hit:87%`. Differs from the existing `cache:` section (which shows cache_read as a share of *total* token traffic dominated by output): `hit:` answers "is my prompt structure cache-friendly?" rather than "is the cache saving me money?" Both can be enabled simultaneously without overlap. Computed from existing normalized fields (`cache_read_input_tokens`, `cache_creation_input_tokens`, `input_tokens`) so this is a zero-new-IO section. Opt-in via custom theme.
...
- 30 new tests: 9 for the 2.1.139 width regression (`TestClaudeCode2139WidthRegression` — covers `COLUMNS=0` distinct from unset, `tput cols=80` stub rejection with a non-stub control case, process-tree walk shape on Windows and POSIX, report-list invariants, thin-wrapper signature preservation), 6 for `fmt_cache_hit_ratio` (zero / perfect / mixed / no-hits / None handling / end-to-end render), 15 for `_count_activity_from_transcript` and `get_session_activity_count` (happy path / zero / no user in tail / missing path / None path / non-string path / empty file / malformed JSON lines / tail-read cap verification with a > 64 KiB file / partial first line discard / TTL cache behavior / end-to-end render with and without `transcript_path`).
...
### Notes
- **Backward compatible** — every existing theme/section keeps working unchanged. The new sections (`activity`, `cache_hit`) are opt-in via custom theme. The width-detection fix is transparent: users on Claude Code < 2.1.139 see no behavior change; users on 2.1.139+ see correct layouts instead of squeezed 80-col ones.
- All 400 tests pass (was 370, +30 new). Pure stdlib, no dependency changes.
