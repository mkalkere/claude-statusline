...
- All 500 tests pass (was 477, +23 net new in `tests/test_ultra_effort.py`: alias-map strict equality with explanatory docstring, ultra at all three layers, stale-cache rehydration scenario, dual-namespace precedence including the both-populated and per-field empty cases, shared PR-number cap on both branches, `workspace.repo` composition, `pr.review_state` capture + rejection, and the workspace isinstance guard at both `_normalize` sites). Pure stdlib, no dependency changes.
