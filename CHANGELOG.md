...
- 744 tests pass (+16: a width-headroom sweep across seven terminal widths, a regression pin reproducing the reported session shape, the three-section promotion, a no-section-on-two-lines invariant across every theme, exact-composition pins for `minimal`/`focus`, and the model shortener's raw-vs-friendly guard). Pure stdlib, zero dependencies, as always.
