...
- All 530 tests pass (was 507, +23 net new in `tests/test_v070_nlines_and_thresholds.py`: 6 for N-line render contract including gap/empty/all-built-in-themes-unchanged, 7 for the `_parse_cc_version()` parser, 7 for `_layout_thresholds()` gate logic, and 3 end-to-end render tests pinning the section-recovered-on-120-cols-with-gates and section-still-dropped-without-gates contracts). Pure stdlib, no dependency changes.
