...
- All 505 tests pass (was 477, +28 net new in `tests/test_ultra_effort.py`: alias-map strict equality with explanatory docstring, ultra at all three layers, stale-cache rehydration scenario, stdin-overrides-stale-cache convergence, dual-namespace precedence including the both-populated and per-field empty cases, shared PR-number cap on both branches with boundary cases at 0 / negative / 999_999, `workspace.repo` composition with empty-string and partial-fallthrough variants, `pr.review_state` capture + rejection, and the workspace isinstance guard at both `_normalize` sites). Pure stdlib, no dependency changes.
